...
import bip
from bip.plugins.juliet.signal_data_packet import _SignalDataPacket as DataPacket
...
def simple_data_packet():
    with io.BytesIO() as f:
        ...
        f.write(struct.pack("<I", 0xDEADBEEF)) #stream id
        ...
        f.write(struct.pack("<I", 0xFFFFFFFF)) #trailer
...
def test_data_packet(simple_data_packet):
    packet = DataPacket(simple_data_packet[1])
    ...
    assert packet.trailer == 0xFFFFFFFF
...
def test_data_packet_multiple_data(data_packet_multiple):
    packet = DataPacket(data_packet_multiple[1])
    assert np.array_equal(
            packet.data,
            np.array([
                [0x0c0d, 0x0a0b],
...
def test_data_packet_notrailer(data_packet_notrailer):
    packet = DataPacket(data_packet_notrailer[1])
